export const fr = {
  language: {
    label: 'Langue',
  },
  navigation: {
    stack: {
      rsvp: 'RSVP',
      login: 'Accès mariés',
      register: 'Inscription',
      notFound: 'Page introuvable',
    },
    menu: {
      primary: 'Navigation principale',
      open: 'Ouvrir le menu',
      close: 'Fermer le menu',
    },
    userMenu: {
      openLabel: 'Menu compte',
      coupleLogin: 'Marié·e ? Connecte-toi ici',
      sectionAccount: 'Compte',
      sectionPreferences: 'Préférences',
    },
    tabs: {
      home: 'Accueil',
      admin: 'Admin',
      album: 'Album',
      travel: 'Contacts',
      tema: 'Le Thème',
      profile: 'Profil',
    },
  },
  common: {
    roles: {
      user: 'Invité',
      admin: 'Admin',
    },
    yes: 'Oui',
    no: 'Non',
    back: 'Retour',
    loading: 'Chargement...',
    loadingSession: 'Chargement de la session...',
    refresh: 'Actualiser',
    refreshing: 'Actualisation...',
    save: 'Enregistrer',
    saving: 'Enregistrement...',
    create: 'Créer',
    update: 'Mettre à jour',
    edit: 'Modifier',
    cancel: 'Annuler',
    delete: 'Supprimer',
    open: 'Ouvrir',
    signOut: 'Se déconnecter',
    bytes: 'octets',
    active: 'Actif',
    hidden: 'Masqué',
    pending: 'En attente',
    approved: 'Approuvée',
    rejected: 'Refusée',
    notAssigned: 'Non attribuée',
    fields: {
      email: 'E-mail',
      password: 'Mot de passe',
      firstName: 'Prénom',
      lastName: 'Nom',
      invitationToken: "Token d'invitation",
      dietaryNotes: 'Notes alimentaires',
      notes: 'Notes',
      address: 'Adresse',
    },
  },
  contactActions: {
    call: 'Appeler',
    whatsapp: 'WhatsApp',
    email: 'E-mail',
    website: 'Site web',
    instagram: 'Instagram',
    facebook: 'Facebook',
    tiktok: 'TikTok',
  },
  factions: {
    scout_regiment: 'Bataillon d’exploration',
    military_police: 'Brigade spéciale',
    garrison: 'Garnison',
  },
  logisticsCategories: {
    location: 'Lieu & Réception',
    beauty: 'Beauté',
    transfer: 'Transport',
    catering: 'Traiteur',
    laundry: 'Blanchisserie',
  },
  photoTags: {
    ceremony: 'Cérémonie',
    cake: 'Découpe du Gâteau',
    party: 'Selfies & Piste',
    toast: 'Toast',
    banquet: 'Banquet',
    speech: 'Discours',
    other: 'Autre',
  },
  landing: {
    nav: {
      story: 'Nous',
      ceremony: 'Cérémonie',
      rsvp: 'RSVP',
      gift: 'Cadeau',
      profile: 'Profil',
      login: 'Connexion',
      admin: 'Admin',
      gallery: 'Galerie',
      faq: 'FAQ',
      contacts: 'Contacts',
    },
    hero: {
      eyebrow: 'Mariage inspiré par Attack on Titan',
      operationTag: 'Opération Pirulini',
      button: 'RSVP ici',
    },
    mission: {
      subtitle: 'La plus belle bataille est celle que l’on mène ensemble',
      scrollCue: 'Offrez votre cœur',
    },
    story: {
      photoOne: 'Photo du couple 01',
      photoTwo: 'Photo du couple 02',
      groomCaption: 'Le Marié',
      brideCaption: 'La Mariée',
      heading: 'Notre Histoire',
      paragraphOne:
        'Ilaria et Davide créent une page qui raconte leur journée de manière plus intime, élégante et personnelle.',
      paragraphTwo:
        'L’idée est de réunir le langage d’un site de mariage éditorial avec une navigation moderne, un RSVP numérique et des informations organisées au même endroit.',
      temaLink: 'Découvrir le thème →',
    },
    ceremony: {
      heading: 'Participez à leur cérémonie intime',
      city: 'Ravenne',
      venue: 'Lido Adriano · Amarissimo Cala Celeste',
      body: 'Cérémonie en bord de mer avec vue sur la côte ravennate. Nous avons hâte de célébrer ce jour avec vous.',
      artworkPlaceholder: 'Photo principale / illustration du couple',
    },
    rsvp: {
      eyebrow: 'Réponse mission',
      heading: 'Confirmez votre présence',
      body: 'Une réservation par famille ou groupe : jusqu’à 10 invités, avec menu et intolérances pour chaque personne.',
      button: 'Ouvrir le formulaire RSVP',
      stepOneLabel: 'Présence',
      stepOneDesc: 'Indiquez si vous participerez — oui ou non — avant la date limite.',
      stepTwoLabel: 'Votre groupe',
      stepTwoDesc: 'Ajoutez les invités avec noms, menus et notes pour la cuisine.',
      stepThreeLabel: 'Faction',
      stepThreeDesc: 'À la confirmation, vous recevez le régiment attribué à votre groupe.',
      deadlineNote: 'Modifications ouvertes jusqu’au 6 mai 2027',
      visualTag: 'Briefing opérationnel',
      visualTitle: 'Party RSVP',
      visualCaption: 'Un compte, une réservation, toute la famille ensemble.',
    },
    gift: {
      eyebrow: 'Notre rêve',
      title: 'Direction le Japon',
      intro:
        'Votre présence est le plus beau cadeau que nous puissions recevoir. Si vous souhaitez nous faire un petit geste en plus, une contribution nous aidera à vivre la lune de miel dont nous rêvons : temples anciens, paysages magnifiques et beaucoup de ramen à partager.',
      gratitude: 'Chaque attention, petite ou grande, sera accueillie avec une profonde gratitude.',
      coordinatesTitle: 'Coordonnées bancaires',
      accountHolder: 'Titulaire du compte',
      iban: 'IBAN',
      bic: 'BIC/SWIFT',
      reference: 'Libellé du virement',
      copyIban: "Copier l'IBAN",
      copiedIban: 'IBAN copié',
    },
    gallery: {
      eyebrow: 'Nos souvenirs',
      title: 'Notre galerie',
      intro: 'Un trésor de moments partagés : jetez un œil aux photos envoyées par vos proches.',
      loading: 'Chargement des photos...',
      empty: 'Pas encore de photos. Les premières arrivent bientôt !',
      error: 'Impossible de charger les photos. Réessayez plus tard.',
      retry: 'Réessayer',
      ctaTitle: 'Envie de revivre ces moments ?',
      ctaBody: 'Téléversez vos photos de notre journée dans la galerie partagée.',
      ctaButton: 'Ouvrir la galerie',
    },
    devTools: {
      title: 'Zone RSVP et outils',
      description:
        'Cette partie reste pensée pour le développement et les tests, mais elle est maintenant intégrée dans une page d’accueil plus ordonnée.',
      generate: {
        title: 'Générer une invitation rapide',
        placeholder: "Nom de l'invité pour générer le token",
        button: 'Générer le token et ouvrir le RSVP',
        loadingButton: "Génération de l'invitation...",
        hint: 'Cette action est réservée aux comptes admin autorisés.',
        emptyNameError: "Saisissez un nom avant de générer l'invitation.",
        createError: 'La création de l’invitation a échoué. Vous devez être connectée/e en admin.',
      },
      open: {
        title: 'Ouvrir une invitation existante',
        placeholder: "Collez ici votre token d'invitation",
        button: "Ouvrir l'écran RSVP",
        demoLink: 'Ouvrir le lien de démonstration de développement',
      },
    },
    faq: {
      eyebrow: 'FAQ',
      title: 'Questions fréquentes',
      locationQuestion: 'Le lieu est-il accessible ?',
      locationAnswer:
        'Oui, toutes les informations pratiques seront communiquées en temps voulu, y compris pour tout besoin d’accessibilité.',
      foodQuestion: 'Comment signaler des allergies ou besoins alimentaires ?',
      foodAnswer:
        'Vous pourrez le faire directement dans le formulaire RSVP, afin que toutes les notes restent liées à votre invitation personnelle.',
      phoneQuestion: 'Puis-je utiliser mon téléphone pendant la cérémonie ?',
      phoneAnswer:
        'Nous serions ravis que vous profitiez de l’instant avec nous — un photographe dédié sera là pour l’immortaliser.',
    },
    decorative: {
      wings: 'Ailes de la liberté',
      mission: 'Journal de mission',
      routes: 'Murs et itinéraires',
    },
    contacts: {
      title: 'Contacts utiles',
      teamTitle: 'Équipe mariage',
      teamLine: 'Davide & Ilaria',
      teamBody: 'davide.ilaria@exemple.fr',
      travelTitle: 'Assistance voyage',
      travelLine: 'Hôtels, déplacements, parkings',
      ceremonyTitle: 'Cérémonie',
      ceremonyLine: "Détails du lieu et des accès",
    },
  },
  auth: {
    experience: {
      stampDate: '2027.05.31',
      login: {
        seriesTitle: 'OPERATION BEYOND THE WALLS',
        stampLabel: 'LOGIN',
        eyebrow: 'Accès mission',
        headline: 'Entrez dans l’opération.',
        lead: 'Un compte pour le RSVP, l’album partagé et les infos du mariage.',
        featureOne: 'Confirmez présence et accompagnants en une fois',
        featureTwo: 'Modifiez votre RSVP jusqu’à la date limite',
        featureThree: 'Publiez vos photos dans l’album après la cérémonie',
        missionCode: 'OP · RAVENNA · HQ',
      },
    },
  },
  login: {
    eyebrow: 'Connexion',
    title: 'Connectez-vous au site.',
    subtitle: 'Saisissez le code secret pour accéder en tant que mariés.',
    sectionLabel: 'Qui êtes-vous ?',
    secretLabel: 'Code secret',
    rememberMe: 'Rester connecté',
    submitLabel: 'Se connecter',
    submitLoading: 'Connexion en cours...',
    genericError: 'Connexion impossible.',
    validation: {
      secretRequired: 'Saisissez le code secret.',
      invalidCredentials: 'Code incorrect.',
    },
    unauthorizedAdminError:
      'Cet accès est réservé à la mariée et au marié. Utilisez un compte autorisé.',
    roleMismatchError: 'Vous avez sélectionné {{selected}}, mais ce compte a un rôle différent.',
    identities: {
      bride: {
        label: 'Mariée',
        title: 'Accès réservé à la mariée',
        description: 'Utilisez le compte autorisé pour entrer dans l’espace privé du mariage.',
        helperText: 'Cet accès nécessite un compte administrateur autorisé.',
        submitLabel: 'Entrer en tant que mariée',
      },
      groom: {
        label: 'Marié',
        title: 'Accès réservé au marié',
        description:
          'Utilisez le compte autorisé pour gérer les contenus, les invités et les détails d’organisation.',
        helperText: 'Cet accès nécessite un compte administrateur autorisé.',
        submitLabel: 'Entrer en tant que marié',
      },
      guest: {
        label: 'Invité',
        title: 'Accès invité',
        description:
          'Connectez-vous avec votre compte pour voir le site, les détails du voyage et les informations utiles.',
        helperText: 'Si c’est votre première visite, vous pouvez aussi créer un compte personnel.',
        submitLabel: 'Entrer en tant qu’invité',
      },
    },
  },
  forgotPassword: {
    title: 'Mot de passe oublié',
    subtitle: 'Indique ton email : si un compte correspond, on t’envoie un lien pour définir ton mot de passe.',
    submitLabel: 'Envoyer le lien',
    submitLoading: 'Envoi en cours...',
    successMessage: 'Si cette adresse correspond à un compte, vérifie ta boîte mail : le lien reste valable une heure.',
    backToLogin: 'Retour à la connexion',
    genericError: 'Envoi impossible. Réessaie.',
  },
  resetPassword: {
    title: 'Définis ton mot de passe',
    subtitle: 'Choisis un nouveau mot de passe pour ton compte.',
    passwordLabel: 'Nouveau mot de passe',
    submitLabel: 'Enregistrer',
    submitLoading: 'Enregistrement...',
    successMessage: 'Mot de passe défini. Tu peux maintenant te connecter avec ton nouveau mot de passe.',
    invalidTokenError: 'Ce lien est invalide ou a expiré. Demandes-en un nouveau.',
    backToLogin: 'Retour à la connexion',
  },
  profile: {
    eyebrow: 'Profil',
    guestTitle: 'Connectez-vous pour enregistrer votre compte.',
    guestSubtitle:
      'Depuis ici, vous pouvez vous inscrire, entrer dans l’app et garder votre session active entre deux visites.',
    loginLink: 'Aller à la connexion',
    registerLink: 'Créer un nouveau compte',
    subtitle:
      'Ici, vous pouvez voir votre rôle, mettre à jour votre prénom et votre nom, et fermer la session quand vous le souhaitez.',
    updatedMessage: 'Profil mis à jour.',
    updateError: 'Mise à jour impossible.',
    updateButton: 'Mettre à jour le profil',
    updateLoading: 'Enregistrement...',
    roleLabel: 'Rôle',
    signOut: 'Se déconnecter',
  },
  rsvp: {
    loadError: 'Impossible de charger votre statut RSVP.',
    invalidLink: "Lien d'invitation invalide.",
    notFound: 'Invitation introuvable ou expirée.',
    alreadyConfirmedError: 'Le RSVP avait déjà été confirmé auparavant.',
    submitError: 'La confirmation a échoué. Réessayez dans un instant.',
    deadlineClosedError: 'Les modifications RSVP sont closes.',
    eyebrow: 'RSVP',
    guestFallbackName: 'Invité',
    subtitle:
      'Indiquez qui participera et les préférences de menu pour chaque personne. La faction est attribuée automatiquement à la confirmation.',
    confirmedTitle: 'RSVP confirmé',
    statusLabel: 'Statut',
    attendingStatus: 'Présent',
    notAttendingStatus: 'Absent',
    factionLabel: 'Faction attribuée',
    dietaryLabel: 'Notes alimentaires',
    dietaryNotesLabel: 'Autres indications',
    dietaryNotesPlaceholder: 'Allergies hors liste, menu enfant, demandes pour la cuisine...',
    formTitle: 'Serez-vous présent ?',
    loginHint: 'Vous devez d’abord vous connecter avec votre compte pour confirmer le RSVP.',
    loginLink: 'Aller à la connexion',
    attendQuestion: 'Participerez-vous ?',
    notAttendingHint: 'Si vous sélectionnez Non, il n’est pas nécessaire d’indiquer les participants.',
    submitLabel: 'Confirmer le RSVP',
    submitLoading: 'Envoi en cours...',
    saveEditLabel: 'Enregistrer les modifications',
    saveEditLoading: 'Enregistrement...',
    editButton: 'Modifier la réponse',
    loginSubmitLabel: 'Connectez-vous pour confirmer le RSVP',
    guestsTitle: 'Participants',
    guestsHint: 'La première ligne correspond au compte avec lequel vous êtes connecté.',
    guestsSummaryTitle: 'Détail des participants',
    guestLabel: 'Participant {{number}}',
    accountHolderBadge: 'Compte enregistré',
    partyCount: '{{current}} / {{max}}',
    phaseForm: 'Phase 01 — Saisie',
    phaseSummary: 'Phase 02 — Récapitulatif',
    addGuest: '+ Ajouter un participant',
    removeGuest: 'Retirer',
    firstNameLabel: 'Prénom',
    lastNameLabel: 'Nom',
    mealLabel: 'Type de menu',
    intoleranceLabel: 'Intolérances',
    isChildLabel: 'Est un enfant',
    maxGuestsReached: 'Vous avez atteint le maximum de 10 participants par réservation.',
    deadlineClosedTitle: 'Modifications closes',
    deadlineClosedBanner:
      'Les modifications sont closes : vous pouvez consulter le récapitulatif mais pas mettre à jour la réservation.',
    cancelEditLabel: 'Annuler les modifications',
    mealChoices: {
      standard: 'Standard',
      vegetarian: 'Végétarien',
      vegan: 'Végan',
      gluten_free: 'Sans gluten',
      baby: 'Menu enfant',
    },
    intolerances: {
      none: 'Aucune',
      gluten: 'Gluten',
      lactose: 'Lactose',
      eggs: 'Œufs',
      nuts: 'Fruits à coque',
      seafood: 'Poisson / crustacés',
      other: 'Autre (préciser dans les notes)',
    },
    validation: {
      firstNameRequired: 'Saisissez le prénom du participant.',
      lastNameRequired: 'Saisissez le nom du participant.',
      otherNotesRequired: 'Précisez l’intolérance dans le champ notes.',
    },
  },
  album: {
    loadError: "Impossible de charger l'album pour le moment.",
    galleryPermissionError:
      'Autorisation galerie refusée. Activez-la dans les réglages de votre appareil.',
    missingTokenError: "Saisissez le token d'invitation avant d'envoyer une photo.",
    missingPhotoError: "Sélectionnez d'abord une photo dans la galerie.",
    uploadSuccess: 'Photo envoyée et publiée dans l’album.',
    uploadError: 'Envoi impossible. Vérifiez la connexion.',
    filesSelectedCount: '{{count}} fichiers sélectionnés',
    uploadButtonMultiple: 'Envoyer {{count}} fichiers dans l’album',
    uploadProgress: 'Envoi {{done}}/{{total}}...',
    uploadSuccessMultiple: '{{count}} fichiers envoyés et publiés dans l’album.',
    uploadPartialError: '{{done}}/{{total}} fichiers envoyés, certains ont échoué.',
    eyebrow: 'Album',
    title: 'Nous rassemblons les souvenirs du mariage au même endroit.',
    subtitle:
      'Connectez-vous et partagez vos photos : elles apparaissent tout de suite dans l’album.',
    refreshButton: "Actualiser l'album",
    refreshLoading: 'Actualisation...',
    uploadTitle: 'Envoyer une photo',
    uploadDescription:
      'Choisissez une photo sur votre appareil et ajoutez une courte légende si vous le souhaitez.',
    loginHint: 'Pour envoyer une photo, vous devez d’abord vous connecter avec votre compte.',
    captionPlaceholder: 'Légende facultative',
    tagPlaceholder: 'Moment (facultatif)',
    choosePhoto: 'Choisir une photo depuis la galerie',
    changePhoto: 'Changer la photo sélectionnée',
    loginChoosePhoto: 'Connectez-vous pour choisir une photo',
    uploadButton: "Envoyer la photo dans l'album",
    uploadLoading: 'Envoi en cours...',
    loginUploadButton: 'Connectez-vous pour envoyer une photo',
    galleryTitle: 'Photos de l’album',
    galleryEmpty: 'Aucune photo pour le moment. Soyez le premier à partager un souvenir.',
    previousMedia: 'Photo précédente',
    nextMedia: 'Photo suivante',
  },
  travel: {
    loadError: 'Impossible de charger les contacts logistiques.',
    eyebrow: 'Espace voyage',
    title: 'Tous les repères utiles sur un seul écran.',
    subtitle:
      'Ici, vous trouverez les contacts opérationnels pour la préparation, l’hôtel, le transfert et d’autres services utiles pour le mariage.',
    refreshButton: 'Actualiser les contacts',
    refreshLoading: 'Actualisation...',
    emptyTitle: 'Aucun contact disponible',
    emptyBody:
      'Quand l’admin ajoutera les services logistiques, vous les trouverez ici classés par catégorie.',
    contactPerson: 'Contact : {{value}}',
  },
  tema: {
    eyebrow: 'Opération Pirulini',
    title: 'Le thème de notre mariage',
    subtitle:
      'Des ailes de la liberté, un carnet de mission et deux calopsittes qui ont conquis nos cœurs : voilà pourquoi tout ici parle d’Attack on Titan.',
    crestTitle: 'Le blason',
    crestBody:
      'Les ailes du Bataillon d’Exploration entourent nos deux calopsittes : notre petite expédition au-delà des murs, du premier rendez-vous à l’autel.',
    styleTitle: 'Le style',
    styleBody:
      'Chaque page de ce site se lit comme un document de mission — badges, jargon militaire et cartes plutôt que les formules habituelles de mariage — car Attack on Titan est la série qui nous a fait aimer aussi cela.',
    missionTitle: 'La mission',
    missionBody:
      'Le nom « Pirulini » vient de Pirulì, un créateur que nous regardions ensemble : nos calopsittes en ont tiré le surnom « Pirù », et « Opération Pirulini » est devenu le nom de code du jour du oui — une seule bataille à gagner ensemble, entourés des personnes que nous aimons.',
    watchTitle: 'Où la regarder',
    watchIntro:
      'Si vous voulez découvrir (ou revoir) la série avant le mariage, voici où la trouver légalement.',
    watchAnimeLabel: 'Anime — en streaming',
    watchMangaLabel: 'Manga — chapitres officiels gratuits',
    watchNote:
      'Les chapitres gratuits sont limités : c’est le moyen le plus économique de découvrir l’histoire, pas de la lire en entier.',
  },
  admin: {
    nav: {
      rsvp: 'Accueil',
      contacts: 'Contacts',
      gallery: 'Galerie',
    },
    placeholder: {
      comingSoon: 'Section bientôt disponible.',
    },
    photoStatuses: {
      approved: 'Approuvée',
      rejected: 'Refusée',
      pending: 'En attente',
    },
    rsvpStatuses: {
      pending: 'En attente',
      attending: 'Participe',
      notAttending: 'Ne participe pas',
    },
    errors: {
      loginRequired: 'Connectez-vous avec votre compte pour ouvrir le tableau de bord admin.',
      notAuthorized: 'Cette section est visible uniquement pour les comptes admin autorisés.',
      loadFailed: 'Impossible de charger le tableau de bord admin.',
      contactTitleRequired: 'Saisissez au moins un titre pour le contact.',
      contactSaveFailed: 'Enregistrement du contact impossible.',
      contactUpdateFailed: 'Mise à jour du contact impossible.',
      contactDeleteFailed: 'Suppression du contact impossible.',
      photoUpdateFailed: 'Mise à jour de la photo impossible.',
      photoDeleteFailed: 'Suppression de la photo impossible.',
    },
    auth: {
      loginFromProfile: 'Connectez-vous depuis le profil pour utiliser le tableau de bord admin.',
      notAuthorized: 'Zone réservée aux comptes admin autorisés.',
    },
    hero: {
      eyebrow: 'Admin',
      title: 'Tableau de bord.',
      subtitle:
        'Ici, vous suivez les invités, les factions et mettez à jour le carnet de contacts utile pour le mariage.',
      refresh: 'Actualiser le tableau de bord',
      refreshing: 'Actualisation...',
      rsvpCode: 'Code 31.05.2027',
      rsvpTitleLead: 'Tableau de bord',
      rsvpTitleHighlight: 'Présences',
      rsvpSubtitle:
        'Suivi des présences en temps réel, gestion des tables, accompagnants et besoins alimentaires pour le banquet de mariage de Davide & Ilaria.',
      contactsCode: 'Logistique 24h',
      contactsTitleLead: 'Répertoire Fournisseurs',
      contactsTitleHighlight: '& Logistique',
      contactsSubtitle:
        'Coordination stratégique, fournisseurs confirmés, points de livraison et contacts prioritaires pour le mariage de Davide & Ilaria.',
      galleryEyebrowLiveCloud: 'Live Cloud',
      galleryEyebrowArchive: 'Archives Tactiques',
      galleryTitleLead: 'Galerie Photo',
      galleryTitleHighlight: '& Souvenirs',
      gallerySubtitle:
        'Contrôle opérationnel et modération en temps réel des photos envoyées par les invités pour le mariage de Davide & Ilaria.',
    },
    stats: {
      invited: 'Utilisateurs inscrits',
      users: 'Utilisateurs inscrits',
      confirmed: 'Confirmés',
      attending: 'Présents',
      notAttending: 'Absents',
      confirmedCard: 'Confirmés',
      pendingCard: 'En attente',
      declinedCard: 'Refusés',
      totalSeatsCard: 'Couverts totaux',
      guestsUnit: 'invités',
      pendingUnit: 'à confirmer',
      declinedUnit: 'absents',
      seatsUnit: 'couverts',
      confirmedCardSubtitle: '{{percent}}% des invités',
      pendingCardSubtitle: '{{percent}}% des invités',
      declinedCardSubtitle: '{{percent}}% des invités',
      totalSeatsCardSubtitle: '{{count}} enfants inclus',
    },
    rsvpEntries: {
      title: 'Invités confirmés',
      searchLabel: 'Filtrer par invité',
      searchPlaceholder: 'Rechercher par nom ou régime alimentaire...',
      noCompanions: 'Aucun accompagnant',
      filterAll: 'Tous ({{count}})',
      filterSpecialDiet: 'Régimes spéciaux ({{count}})',
      filterChildren: 'Enfants ({{count}})',
      confirmedBadge: 'Confirmé',
      companionsLabel: 'Accompagnant : {{value}}',
      childTag: 'Enfant',
      notesLabel: 'Note : {{value}}',
      empty: 'Aucun invité confirmé pour le moment.',
      loadFailed: 'Impossible de charger les invités confirmés.',
      pageOf: 'Page {{page}} sur {{totalPages}}',
      prevPage: 'Préc.',
      nextPage: 'Suiv.',
      menuSummaryTitle: 'Résumé du menu',
      childrenSummaryLabel: 'Enfants',
      sectionTitle: 'Liste des invités & attribution des tables',
      viewedOfConfirmed: '{{shown}} sur {{total}} confirmés affichés',
      assignTableButton: 'Attribuer une table',
      tableAssignNone: '— Aucune table —',
      tableGroupsTitle: 'Répartition des tables',
      tableGroupsEmpty: 'Aucune table créée pour le moment.',
      tableUnassignedLabel: 'Non attribué',
      tableSeatsOf: '{{assigned}} / {{capacity}} places ({{percent}}%)',
      tableSeatsOfComplete: '{{assigned}} / {{capacity}} places (Complet)',
      tableSeatsKicker: 'Places Attribuées',
      newTableCardTitle: 'Nouvelle Table',
      kitchenKicker: 'Cuisine Royale',
      addTableButton: 'Ajouter une table',
      newTableLabelPlaceholder: 'Nom de la table',
      newTableCapacityPlaceholder: 'Places',
      newTableNotePlaceholder: 'Note (facultatif)',
      newTableSubmit: 'Créer une table',
      deleteTableConfirm: 'Supprimer la table',
      deleteTableConfirmBody: 'Supprimer « {{label}} » ? Cette action est irréversible.',
      tableDeleteFailed: 'Impossible de supprimer la table.',
    },
    factions: {
      title: 'Factions',
      empty: 'Aucune faction attribuée pour le moment.',
    },
    photos: {
      title: 'Galerie',
      statsPhotosLabel: 'Photos Invités',
      statsPhotosSubtitle: 'Synchronisées',
      statsVideosLabel: 'Vidéos HD',
      statsVideosSubtitle: 'Clips 4K/1080p',
      statsStorageLabel: 'Stockage Cloud',
      statsStorageUnit: '/ 100 Go',
      filterAll: 'Toutes ({{count}})',
      searchLabel: 'Filtrer les photos',
      searchPlaceholder: 'Rechercher par invité ou légende...',
      liveApprovalLabel: 'Approbation en direct',
      liveApprovalSubtitle: 'Publication auto sur grand écran',
      comingSoon: 'Bientôt disponible',
      zipArchiveButton: 'Archive .ZIP',
      liveSlideButton: 'Démarrer le Live Slide',
      uploadOfficialButton: 'Charger des photos officielles',
      liveBannerTitle: 'Canal Invités Actif :',
      liveBannerIntro: 'Téléversements instantanés activés via le QR Code « Opération Pirulini ».',
      liveBannerLastUpload: 'Dernier envoi {{time}} par {{name}}.',
      liveBannerNoUploads: 'En attente du premier envoi.',
      liveBannerTag: 'Flux en Direct • Mur Rose',
      backupBannerTitle: 'Intégrité Sauvegarde Cloud',
      backupBannerBadge: 'Chiffrement AES-256',
      backupBannerBody:
        '{{used}} Go sur {{quota}} Go utilisés. Les fichiers RAW/HD originaux sont protégés et synchronisés en temps réel avec l\'album photo officiel et le projecteur de la salle.',
      syncLogButton: 'Journal de Synchronisation',
      empty: 'Aucune photo envoyée par les invités.',
      loadFailed: 'Impossible de charger la galerie.',
      uploadedAt: 'Envoyée le {{value}}',
      approvedAt: 'Approuvée le {{value}}',
      approve: 'Approuver',
      backToPending: 'Remettre en attente',
      reject: 'Refuser',
      delete: 'Supprimer',
      deleting: 'Suppression...',
      confirmDelete: 'Supprimer cette photo ? Cette action est irréversible.',
      statusPublished: 'Publiée',
      favoriteButton: 'Ajouter aux favoris',
      unfavoriteButton: 'Retirer des favoris',
      pageOf: 'Page {{page}} sur {{totalPages}}',
      prevPage: 'Précédente',
      nextPage: 'Suivante',
      viewingCount: 'Affichage {{from}}–{{to}} sur {{total}} éléments',
      addPhotoCardLabel: 'Téléverser une photo',
      addPhotoModalTitle: 'Téléverser une photo ou une vidéo',
      favoriteUpdateFailed: 'Impossible de mettre à jour le favori.',
      editButton: 'Modifier',
      editModalTitle: 'Modifier la photo',
      editCaptionPlaceholder: 'Légende',
      editTagPlaceholder: 'Moment (facultatif)',
      editSave: 'Enregistrer',
      editSaving: 'Enregistrement...',
      editFailed: "Impossible d'enregistrer les modifications.",
      previousMedia: 'Photo précédente',
      nextMedia: 'Photo suivante',
    },
    contacts: {
      categoryLabel: 'Catégorie de Service',
      confirmDelete: 'Supprimer ce contact ? Cette action est irréversible.',
      formEyebrow: 'Registre Stratégique',
      newTitle: 'Nouveau Contact Prestataire',
      editTitle: 'Modifier le Contact Prestataire',
      description: "Ajoutez ou mettez à jour une référence opérationnelle pour l'équipe du mariage.",
      fields: {
        supplierName: 'Nom du Prestataire / Entreprise',
        contactPerson: 'Nom du Référent',
        phone: 'Téléphone / WhatsApp',
        arrival: "Horaire d'Arrivée et Lieu",
        notes: 'Notes Opérationnelles & Logistique de Déchargement',
      },
      placeholders: {
        supplierName: 'Ex. Floreal Design Firenze',
        contactPerson: 'Référent sur place',
        phone: '+39 3XX XXXXXXX',
        arrival: 'Ex. 13h30 • Portail Ouest du Lieu',
        notes: 'Détails accès, pass déchargement fourgon, allergies équipe, contact de secours...',
      },
      saveLoading: 'Enregistrement...',
      createButton: 'Enregistrer le Prestataire',
      updateButton: 'Mettre à Jour le Prestataire',
      cancelEdit: 'Annuler la modification',
      activeToggle: 'Entrée active',
      inactiveToggle: 'Entrée désactivée',
      supplierRoleDefault: 'Référent',
      supplierRoleTransfer: 'Chauffeur assigné',
      createdMessage: 'Contact créé.',
      updatedMessage: 'Contact mis à jour.',
      searchLabel: 'Filtrer par prestataire',
      searchPlaceholder: 'Rechercher un prestataire ou un contact...',
      filterAll: 'Tous ({{count}})',
      statsActive: 'Actifs',
      statsActiveSubtitle: '{{percent}}% des prestataires',
      statsInactive: 'Inactifs',
      statsInactiveSubtitle: '{{percent}}% des prestataires',
      empty: 'Aucun contact créé pour le moment.',
      edit: 'Modifier',
      hide: 'Masquer',
      activate: 'Activer',
      delete: 'Supprimer',
    },
    users: {
      title: 'Utilisateurs inscrits',
      empty: 'Aucun utilisateur inscrit.',
    },
    guests: {
      title: 'Invités',
      empty: 'Aucun invité créé pour le moment.',
      token: 'Token : {{value}}',
      faction: 'Faction : {{value}}',
    },
  },
  notFound: {
    title: "Cette page n'existe pas.",
    body: "Le lien peut être incomplet ou la page n'a pas encore été créée.",
    backHome: "Retour à l'accueil",
  },
  invite: {
    greeting: 'Chère/cher {{firstName}},',
    headline: 'Avec joie, nous vous invitons',
    coupleNames: 'Ilaria & Davide',
    ceremonyStart: 'Nous vous attendons pour 17h00, début de la cérémonie.',
    intro:
      "Votre présence à nos côtés compterait beaucoup pour nous. Venez comme vous êtes, avec l'envie de danser jusqu'au bout de la nuit.",
    rsvpSection: {
      title: 'Faites-le nous savoir',
      note: 'Merci de confirmer votre présence avant le 6 mai 2027.',
      yes: 'Nous serons là',
      contact: 'Écrivez-nous',
    },
    moreInfo: {
      text: 'Pour plus d’informations, retrouvez tout ici :',
      cta: 'Aller sur le site',
    },
    openAria: "Ouvrir l'invitation",
    tapHint: "Touchez le sceau pour l'ouvrir",
    notFoundTitle: 'Invitation introuvable.',
    notFoundBody: "Ce lien a peut-être expiré ou est incorrect.",
    notFoundBackHome: 'Aller au site du mariage',
  },
  guestRsvp: {
    intro: 'Confirmez votre présence et celle de votre groupe.',
    confirmedTitle: "Merci, c'est noté.",
    confirmedBody: 'Votre confirmation est enregistrée. Rouvrez ce même lien à tout moment pour revenir ici.',
  },
} as const;
