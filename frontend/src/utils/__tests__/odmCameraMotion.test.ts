...
import { ODM_CAMERA_ANCHORS, ODM_CAMERA_LEGS } from '@/src/data/odmCameraAnchors';
import {
...
describe('odmCameraMotion', () => {
  ...
  it('resolves pose at progress zero to the first anchor', () => {
    const position = new Vector3();
    const target = new Vector3();
    resolveOdmCameraPose(ODM_CAMERA_LEGS, 0, {
      position,
      target,
      roll: 0,
      fov: DEFAULT_ODM_CAMERA_TUNING.baseFov,
    ...
    expect(position.x).toBeCloseTo(ODM_CAMERA_ANCHORS[0]!.position.x, 3);
    expect(position.y).toBeCloseTo(ODM_CAMERA_ANCHORS[0]!.position.y, 3);
    expect(position.z).toBeCloseTo(ODM_CAMERA_ANCHORS[0]!.position.z, 3);
  });
