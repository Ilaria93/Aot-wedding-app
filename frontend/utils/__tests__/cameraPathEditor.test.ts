...
describe('cameraPathEditor', () => {
  ...
  it('resolves active camera path segment from global progress', () => {
    expect(resolveCurrentCameraPathSegment(0).segmentName).toBe('rooftops');
    expect(resolveCurrentCameraPathSegment(0.3).segmentName).toBe('giantWalls');
    expect(resolveCurrentCameraPathSegment(0.6).segmentName).toBe('titanCorridor');
    expect(resolveCurrentCameraPathSegment(0.9).segmentName).toBe('finalArena');
    expect(resolveCurrentCameraPathSegment(0.375).localProgress).toBeCloseTo(0.5, 5);
  });
});
